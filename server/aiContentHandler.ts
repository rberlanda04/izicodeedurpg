// Shared implementation used by the local Vite dev-server proxy
// (vite.config.ts) so AI content generation works today without waiting on
// a Firebase Blaze plan upgrade. The production path is the Cloud Function
// at functions/src/index.ts, which mirrors this same logic — if you change
// the prompts/model here, update both. The NVIDIA API key never leaves
// this server-side module in either path.
...
export type ContentKind = 'quest' | 'curiosity' | 'guildIdentity';
...
/**
 * Prompt instructions alone aren't reliable enough for fields the UI's
 * type system depends on (e.g. sdgGoals feeding fixed badge lookups) — an
 * LLM can and does drift from the requested format. This clamps/repairs
 * the model's output to the shapes the rest of the app actually expects,
 ...
 * array) in `raw`, by counting brace/bracket depth rather than a greedy
 * regex — a greedy `\{[\s\S]*\}` match breaks the moment the model appends
 * anything after a complete object (explanatory text, a second example),
 * which happens in practice even with an explicit single-object
 * instruction. If the found value is an array, unwraps to its first
 * element, since that's the one case observed so far (the model ignoring
 * "single object" and brainstorming a list of options instead).
 */
...
export async function generateContent(
  ...
  if (!systemPrompt) {
    throw new Error(`Tipo de conteúdo desconhecido: ${kind}`);
  }
